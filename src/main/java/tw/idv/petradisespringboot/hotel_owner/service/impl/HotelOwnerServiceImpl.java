...
public class HotelOwnerServiceImpl implements HotelOwnerService {
	...
	public List<HotelOwnerVO> findKeyword(String keyword) {
		...
		if (keyword != null) {
			list = list.stream()
					.filter(vo -> vo.getOwnerName().contains(keyword) || vo.getHotelName().contains(keyword) || vo.getOwnerAccount().contains(keyword))
					.collect(Collectors.toList());
		}
		return list;

	}
	public void deleteOwner(Integer hotelId) {
		hotelOwnerRepository.deleteById(hotelId);
	}
}